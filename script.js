...
// JSON key isimleri (python çıktısından kopyalandı)
const K_WORT  = "Wort";
...
const K_KARTE = "Veröffentlichungsdatum \n(Karte)";  // sondaki boşluk yok, baştaki \n var

// GÜN bazlı filtre: bugün ve önceki günlerin kartları görünür
// Saat farkını görmezden gel → günün SONUNU al (23:59:59)
const TODAY_END = (() => {
  const d = new Date();
...
async function loadModul1() {
  try {
    ...
    // Tarih filtresi: Karte tarihi <= bugünün sonu (gün bazlı)
    ...
    // Kaç kapitel yayında?
    const allK  = uniq(m1All.map(r => r[K_KAPI]).filter(Boolean)).sort((a,b)=>a-b);
    const openK = uniq(m1Vocab.map(r => r[K_KAPI]).filter(Boolean)).sort((a,b)=>a-b);
    ...
    console.error("Modul1 Fehler:", e);
    setText("m1-words-display", "Ladefehler!");
    setText("load-hint", "modul1.json konnte nicht geladen werden. Alle Dateien müssen im gleichen Ordner sein und über einen Webserver geöffnet werden (z.B. Live Server).");
  }
}
...
function buildM1Menu() {
  const ks  = uniq(m1Vocab.map(r => r[K_KAPI]).filter(Boolean)).sort((a,b)=>a-b);
  [document.getElementById("f-unit"), document.getElementById("f-part")].forEach(sel => {
    if (!sel) return;
...
function buildM2Menu() {
  ...
  uniq(m2Vocab.map(v => v[M2C.lesson]).filter(x => isFinite(x))).sort((a,b)=>a-b).forEach(l => {
    const o = document.createElement("option");
    o.value = String(l); o.innerText = "Lektion " + l;
    sel.appendChild(o);
  });
}
...
function openTrainer(mod) {
  ...
  if (!vocab.length) {
    alert("Daten noch nicht geladen. Bitte kurz warten und erneut versuchen.");
    return;
  }
  hide("page-menu");
  if (mod === 1) {
    ...
    m2Quiz = false;
    show("m2-flash"); hide("m2-quiz");
    setText("m2-toggle-btn", "🎯 Quiz Modus");
    m2Init();
  }
}
...
function initSession() {
  ...
  let list = unit === "all"
    ? [...m1Vocab]
    : m1Vocab.filter(r => String(r[K_KAPI]) === unit);
...
function doFav() {
  ...
  saveLS();
  const fb = document.getElementById("fav-btn");
  if (fb) fb.textContent = favSet.has(w) ? "★" : "☆";
  updateStatBoxes();
}
...
function renderQuiz(item) {
  ...
  while (opts.length < 4 && g++ < 500) {
    const r = m1Vocab[Math.floor(Math.random() * m1Vocab.length)][K_WORT];
...
function renderWrite(item) {
  ...
  if (res) { res.className = "m1-wresult hidden"; res.innerText = ""; }
}
...
function doCheck() {
  ...
  if (answer === correct) {
    ...
    updateStatBoxes(); setTimeout(doNext, 700);
  } else {
    res.className = "m1-wresult c-err";
    res.innerText = "❌ Falsch – Richtig: " + item[K_WORT];
  }
}
...
function renderReview() {
  ...
  el.innerHTML = list.map((v, i) => {
    const w = esc(v[K_WORT] || "");
    const s = esc(v[K_SENT] || "");
...
function m2SetupQuiz(item, lang) {
  ...
  let g = 0;
  while (opts.length < 4 && g++ < 500) {
    const r = m2Vocab[Math.floor(Math.random() * m2Vocab.length)][lang];
    if (r && !opts.includes(r)) opts.push(r);
  }
  while (opts.length < 4) opts.push("(keine Option)");
  opts.sort(() => Math.random() - .5);
...
function setBtn(id, txt, dis)  { const e = document.getElementById(id); if (e) { e.innerText = txt; e.disabled = dis; } }
